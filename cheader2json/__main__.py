...
@cli.command()
@click.option('-f', '--prefix', help="File name prefix to use for output json files. Inferred from the name of first header file provided if not specified.")
@click.option('--ignore-macro', '-i', envvar="IGNORED_MACROS", multiple=True, help="Macro to ignore. Can be given multiple times (if using an envvar the names are split on spaces).")
# Potentially useful to also support reading from stdin by passing `-` as filename?
@click.argument('header', nargs=-1, required=True, type=click.Path(exists=True, path_type=pathlib.Path))
def convert(header: tuple[pathlib.Path], prefix: Optional[str], ignore_macro: tuple[str]):
...
def diff(oldast, newast):
    """Compare two AST JSON files and print a human-readable set of changes that occured in the header files."""
